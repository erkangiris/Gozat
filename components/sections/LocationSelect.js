...
export default function LocationSelect() {
  ...
  useEffect(() => {
    const currentCity = getSessionStorage('city');
    if (currentCity) {
      setCity(currentCity);
    }
  }, []);
  ...
  const fetchLocationDetails = (latitude, longitude) => {
    ...
    fetch(url)
      ...
      .then(data => {
        ...
        setCity(addressComponents?.address_components[4]?.long_name);
        setSessionStorage('city', addressComponents?.address_components[4]?.long_name);
      })
      .catch(error => {
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} className='flex gap-2 items-center px-4 py-2 bg-gozatgray-200 bg-opacity-70 dark:bg-gray-200 dark:bg-opacity-100 rounded-lg'>
        <Typography variant='textSm' className='font-semibold'>
          {city ? city : 'Şehir Seçin'}
        </Typography>
      ...
      <Dialog open={open} onClose={setOpen} className="relative z-10">
        ...
        <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-10 text-center sm:items-center sm:p-0">
            <DialogPanel
              transition
              className="relative transform overflow-hidden rounded-lg bg-white p-7 text-left shadow-xl transition-all data-[closed]:translate-y-4 data-[closed]:opacity-0 data-[enter]:duration-300 data-[leave]:duration-200 data-[enter]:ease-out data-[leave]:ease-in sm:my-8 sm:w-full sm:max-w-sm sm:p-6 data-[closed]:sm:translate-y-0 data-[closed]:sm:scale-95"
            >
              <div className='w-full flex flex-col max-w-96 justify-center items-center gap-6'>
                ...
                <div className='flex flex-col gap-2 w-full'>
                  ...
                  <Typography variant='textSm' className="text-black">Yakınınızdaki etkinlikler, mekanlar ve diğer herşey için lütfen konum erişimine izin verin</Typography>
                </div>
                <StandartButton title="İzin Ver" className="lg:w-full lg:min-w-full" onClick={() => getLocation()}>izin ver</StandartButton>
                <div className='flex flex-col w-full gap-2'>
                  <div>
                    ...
                    <select
                      ...
                      value={city}
                      defaultValue="Canada"
                      onChange={(e)=>{setCity(e.target.value); setSessionStorage('city', e.target.value)}}
